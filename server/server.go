...
func (serv *Server) handleCommitMsg(content []byte) {
	...
	fmt.Println("server", serv.id, "has ", len(serv.msgbuff.CommitVote[theterm][theorder]), "commit-vote at term ", theterm," height", commitmsg.Order, "the digest is ", commitmsg.Digest)
	serv.msgbuff.Msgbuffmu.Unlock()
}
